...
# Test the augmenter on Kinshuk's multiplexed-sensing TEST set (test-only, no fine-tuning).
# Generates replicates from one test rep per condition, scores realism vs the pooled real
# replicates (all reps, all splits). Compares BASE vs 2sp-fine-tuned vs Rattray-fine-tuned.
set -uo pipefail
...
OUTROOT=$REPO/multiplexed_eval_out
...
score () {  # $1=tag $2=ckpt
  ...
  python eval_metrics.py --gen_dir "$gen" --real_spec "MPLEX:$MPX/reals" --out "$res" || { echo "  eval FAILED"; return; }
